    ...
    public class GraphEditor : GraphView
    {
        ...
        public void PopulateGraph(BaseGraph graph)
        {
            ...
            foreach (BaseNode node in graph.nodes)
            {
                ...
                {
                    BaseNodeEditor parentView = FindNodeView(node);
                    BaseNodeEditor childView = FindNodeView(child);
            ...
            foreach (var node in nodeTypes)
            {
                ...
                if (node.Name != "SingleNode" && node.Name != "MultiNode" && node.Name != "PureNode" && node.Name != "RootNode")
                {
                    if (node.Namespace != null)
                        ...
                        evt.menu.AppendAction($"[{node.BaseType.Name}] {node.Name}", action => CreateNode(node));
                }
            }
        }
        ...
        private BaseNodeEditor FindNodeView(BaseNode node)
        {
            return GetNodeByGuid(node.m_guid) as BaseNodeEditor;
        }
